...
dsQualidade$Media <- rowMeans(ds[,4:66], na.rm=TRUE)
...
ggplot() +
  geom_bar(data = dsQualidadeAgrupPorEmpresaeUF[with(dsQualidadeAgrupPorEmpresaeUF,Nome.Fantasia == 'Oi'),],
...
dsQualidadeInvertidos <- melt(dsQualidadeAgrupPorEmpresaeUF,id=c("Nome.Fantasia","Unidade.Federativa"))
head(dsQualidadeInvertidos)

# Criando o gráfico
ggplot(dsQualidadeInvertidos[with(dsQualidadeInvertidos,Nome.Fantasia == 'Oi'),],
...
#dsQualidade$Nome.Fantasia <- factor(dsQualidade$Nome.Fantasia, levels=c('Claro', 'Oi', 'Tim', 'Vivo'), labels=c(0,1,2,3))
#
#dsQualidade$Unidade.Federativa <- factor(dsQualidade$Unidade.Federativa, levels=c('AC','AL','AM','AP','BA','CE','DF','ES','GO','MA','MG',
...
dsAssinantesAgrupPorEmpresaeUF <- dsAssinantes %>% 
  group_by(Grupo, UF) %>% 
  summarise(jan_2018 = sum(X01.01.2018), fev_2018 = sum(X01.02.2018), mar_2018 = sum(X01.03.2018))
...
dsReclamacoesAgrupado <- dsReclamacoes %>% 
  group_by(Ano, Mes, GrupoEconNorm, UF) %>% summarise(QtdeReclamacoes = sum(QtdeSolic))
summary(dsReclamacoesAgrupado)

dsReclamacoesAgrupadoPorMeses <- distinct(dsReclamacoes,GrupoEconNorm, UF)

dsFiltrado <- dsReclamacoesAgrupado[with(dsReclamacoesAgrupado, Ano == 2018 & Mes == 1), c(3,4,5)]
summary(dsFiltrado)
dsReclamacoesAgrupadoPorMeses <- 
  merge(x=dsReclamacoesAgrupadoPorMeses, y=dsFiltrado, by=c('GrupoEconNorm','UF'), all.x = TRUE)
